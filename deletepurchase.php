...
if (isset($_POST["id"]) and !empty($_POST["id"])){
    ...
    $sql="DELETE FROM `purchaseform` WHERE id='$idvalue'";

    $deleterecord=mysqli_query($link,$sql);

    if ($deleterecord){
        echo "Purchase is deleted";
        header("location:loginview.php");
    }else{
        echo "Purchase is not deleted";
    }
...
<div class="container">
    ...
    <h1 style="text-align: center">Are You Sure You Want To Delete Purchase</h1>

    <form action="deletepurchase.php" method="post">

        <div class="alert alert-danger">
            <input type="hidden" name="id" value="<?php echo $_GET["id"] ?>">
            <p style="text-align: center">Are you sure you want the record of purchae deleted?</p>
            <div style="text-align: center">
            <input type="submit" value="Yes" class="btn bg-danger">
            <a href="WebHomePage.html" class="btn btn-warning">No</a>
            </div>
        </div>
